...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("1️⃣ Overall Data (Full YTD-MTD)", type=['csv', 'xlsx', 'xls'], help="Full YTD-MTD Entity Order Summary from DSP Console")
    uploaded_3day = st.file_uploader("2️⃣ Last 3 Days Data (for DRR)", type=['csv', 'xlsx', 'xls'], help="Same format, filtered to last 3 days")
    st.markdown("---")
...
def extract_budget_from_name(order_name):
    ...
    for match in re.finditer(r'(\d+\.?\d*)\s*([KkLl])', name):
        ...
        if unit == 'L': all_matches.append((position, value * 100000))
        elif unit == 'K': all_matches.append((position, value * 1000))
    if all_matches:
...
def parse_file(uploaded):
    ...
    else: return pd.read_excel(uploaded)
...
def standardize_columns(df):
    ...
    for col in df.columns:
        cl = col.lower().strip()
        mapped_name = None
        if 'order status' in cl: mapped_name = 'Order Status'
        elif 'campaign name' in cl: mapped_name = 'Order Name'
        elif 'advertiser' in cl and 'account' in cl: mapped_name = 'Account'
        elif 'start' in cl and 'date' in cl: mapped_name = 'Start Date'
        elif 'end' in cl and 'date' in cl: mapped_name = 'End Date'
        elif 'budget' in cl: mapped_name = 'Budget'
        elif 'total cost' in cl: mapped_name = 'Total Spend'
        elif cl == 'impressions': mapped_name = 'Impressions'
        elif 'click' in cl: mapped_name = 'Clicks'
        elif cl == 'ctr': mapped_name = 'CTR'
        elif 'total roas' in cl and 'click' not in cl: mapped_name = 'ROAS'
        elif 'total dpvr' in cl: mapped_name = 'DPVR'
        elif 'total purchases' in cl: mapped_name = 'Purchases'
        elif 'ecpm' in cl: mapped_name = 'eCPM'
        elif 'new-to-brand' in cl or 'ntb' in cl: mapped_name = 'NTB'
...
def process_data(df, today, drr_data=None, proj_date=None):
    ...
    df['Budget'] = df.apply(lambda row: extract_budget_from_name(row['Order Name']) if row.get('Budget', 0) == 0 else row['Budget'], axis=1)
...
if uploaded_file is not None:
    ...
    drr_raw = parse_file(uploaded_3day) if uploaded_3day is not None else None
    ...
    if report_name: title_text += f" — {report_name}"
    st.title(title_text)
    st.caption(f"📅 {TODAY.strftime('%d %B %Y')} | Total Orders: {len(df)}")
    if uploaded_3day is not None:
        st.success("✅ 3-Day DRR file loaded")
    else:
        st.info("ℹ️ Upload 'Last 3 Days Data' for accurate Current DRR")
    st.markdown("---")
    ...
    if show_projection:
        ...
        pr4.metric("📉 Projected Gap", f"₹{(total_budget - proj_total_spend)/100000:.1f}L")
    st.markdown("---")

    # ACCOUNT OVERVIEW TABLE
    st.header("🏢 Account-Level Overview")
    if len(acct_df) > 0:
        account_summary = acct_df.groupby('Account Short').agg({'Budget': 'sum', 'Total Spend': 'sum', 'Ideal Spend': 'sum', 'Elapsed Days': 'mean', 'Total Days': 'mean', 'Current DRR': 'sum', 'Expected DRR': 'sum', 'Projected Spend': 'sum', 'CTR': 'mean', 'DPVR': 'mean', 'ROAS': 'mean', 'Order Name': 'count'}).reset_index()
        account_summary.columns = ['Account', 'Budget', 'Spends', 'Ideal Spend', 'Avg Elapsed', 'Avg Total Days', 'Current DRR', 'Expected DRR', 'Projected Spend', 'CTR', 'DPVR', 'ROAS', 'Orders']
        ...
        if show_projection: disp_cols.append('Proj DR %')
        ...
        fmt = {'Budget': '₹{:,.0f}', 'Spends': '₹{:,.0f}', 'DR %': '{:.1f}%', 'Expected DR %': '{:.1f}%', 'Pacing %': '{:.1f}%', 'Current DRR': '₹{:,.0f}', 'Expected DRR': '₹{:,.0f}', 'CTR': '{:.4f}', 'DPVR': '{:.4f}', 'ROAS': '{:.2f}'}
        if show_projection: fmt['Proj DR %'] = '{:.1f}%'
        st.dataframe(acct_display.style.format(fmt), use_container_width=True, height=min(600, max(250, len(account_summary) * 38)))
    st.markdown("---")

    # ORDER-LEVEL TABLE
    st.header("📋 Order-Level Delivery Tracker")
    fcol1, fcol2, fcol3 = st.columns([2, 2, 1])
    with fcol1: acct_filter = st.multiselect("Filter by Account", options=sorted(active_df['Account Short'].unique()), default=[])
    with fcol2: status_filter = st.multiselect("Filter by Status", options=active_df['Status'].unique().tolist(), default=active_df['Status'].unique().tolist())
    with fcol3: pacing_range = st.slider("Pacing %", 0, 200, (0, 200))
    ...
    order_data = {'Account': filtered_df['Account Short'].values, 'Order Name': filtered_df['Order Name'].values, 'Budget': filtered_df['Budget'].values, 'Spends': filtered_df['Total Spend'].values, 'DR %': filtered_df['DR %'].values, 'Expected DR %': filtered_df['Expected DR %'].values, 'Pacing %': filtered_df['Pacing %'].values, 'Current DRR': filtered_df['Current DRR'].values, 'Expected DRR': filtered_df['Expected DRR'].values}
    if show_projection:
    ...
    ofmt = {'Budget': '₹{:,.0f}', 'Spends': '₹{:,.0f}', 'DR %': '{:.1f}%', 'Expected DR %': '{:.1f}%', 'Pacing %': '{:.1f}%', 'Current DRR': '₹{:,.0f}', 'Expected DRR': '₹{:,.0f}', 'CTR': '{:.4f}', 'DPVR': '{:.4f}', 'ROAS': '{:.2f}'}
    if show_projection: ofmt['Projected Spend'] = '₹{:,.0f}'; ofmt['Proj DR %'] = '{:.1f}%'
    ...
    if perf_view == "Account Level":
        ...
        if len(perf_source) > 0:
            perf_agg = perf_source.groupby('Account Short').agg({'CTR': 'mean', 'DPVR': 'mean', 'NTB': 'mean', 'ROAS': 'mean', 'Total Spend': 'sum', 'Order Name': 'count'}).reset_index()
            perf_agg.columns = ['Account', 'CTR', 'DPVR', 'NTB', 'ROAS', 'Spend', 'Orders']
            perf_agg = perf_agg.sort_values('Spend', ascending=False)
            perf_display = pd.DataFrame({'Account': perf_agg['Account'].values, 'CTR': [badge_ctr(v) for v in perf_agg['CTR'].values], 'DPVR': [badge_dpvr(d, c) for d, c in zip(perf_agg['DPVR'].values, perf_agg['CTR'].values)], 'NTB': [badge_ntb(v) for v in perf_agg['NTB'].values], 'ROAS': [badge_roas(v) for v in perf_agg['ROAS'].values], 'Spend': [f"₹{v:,.0f}" for v in perf_agg['Spend'].values], 'Orders': perf_agg['Orders'].values})
        ...
        if perf_acct != "All": perf_orders = perf_orders[perf_orders['Account Short'] == perf_acct]
        if len(perf_orders) > 0:
            perf_ord_display = pd.DataFrame({'Account': perf_orders['Account Short'].values, 'Order Name': perf_orders['Order Name'].str[:55].values, 'CTR': [badge_ctr(v) for v in perf_orders['CTR'].values], 'DPVR': [badge_dpvr(d, c) for d, c in zip(perf_orders['DPVR'].values, perf_orders['CTR'].values)], 'NTB': [badge_ntb(v) for v in perf_orders['NTB'].values], 'ROAS': [badge_roas(v) for v in perf_orders['ROAS'].values], 'Spend': [f"₹{v:,.0f}" for v in perf_orders['Total Spend'].values]})
    ...
    with dl1:
        st.download_button("⬇️ Order Tracker (CSV)", active_df.to_csv(index=False), f"order_tracker_{TODAY.strftime('%Y%m%d')}.csv", "text/csv")
    with dl2:
        if len(acct_df) > 0:
            st.download_button("⬇️ Account Summary (CSV)", account_summary.to_csv(index=False), f"account_summary_{TODAY.strftime('%Y%m%d')}.csv", "text/csv")
    ...
    **File 2 (Optional):** Last 3 Days data — Same format, filtered to last 3 days for accurate DRR
    ...
    If budget is missing in the file, extracted from the **last number** in order name:
